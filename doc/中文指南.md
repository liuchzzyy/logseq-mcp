...
> **版本**: 2.1.1 | **Python**: >=3.12 | **协议**: MCP (Model Context Protocol)
>
> **更新**: 2026-02-08
>
...
| 维度 | 说明 |
|------|------|
| MCP Tools | 25 个工具，覆盖块/页面/编辑/查询/图谱/Git 六大类（部分工具受功能开关控制） |
...
class LogseqSettings(BaseSettings):
    ...
    # 核心配置
    api_token: str | None        # LOGSEQ_API_TOKEN (必填，启动时校验)
    ...
    # 功能开关
    enable_advanced_queries: bool = True      # LOGSEQ_ENABLE_ADVANCED_QUERIES
    enable_git_operations: bool = False       # LOGSEQ_ENABLE_GIT_OPERATIONS
    enable_asset_management: bool = False     # LOGSEQ_ENABLE_ASSET_MANAGEMENT
```

**全局单例**: `settings = LogseqSettings()` 在模块加载时实例化；`LOGSEQ_API_TOKEN` 会在服务启动时校验。
...
- **重试**: Tenacity 异步重试，指数退避（1s → 2s → 4s...，最多 10s）
- **超时**: 默认 10 秒，支持每次请求自定义
- **错误映射**: HTTP 401/403 → AuthenticationError, 4xx/5xx → APIError
...
class BaseAPIClient(ABC):
    ...
    async def _make_request(self, method: str, args: list, timeout=None) -> dict:
...
继承 `BaseAPIClient`，封装 30+ 个 Logseq HTTP API 方法（全部为 async）：
...
`get_tools()` 默认返回基础工具集合，高级查询与 Git 工具受功能开关控制：
- `LOGSEQ_ENABLE_ADVANCED_QUERIES=true`：开启 `logseq_advanced_query` / `logseq_get_tasks`
- `LOGSEQ_ENABLE_GIT_OPERATIONS=true`：开启 `logseq_git_commit` / `logseq_git_status`
...
`handle_tool(name, arguments)` 使用 Python 3.12 的 `match/case` 语法路由：
...
uv run logseq-mcp pages create --name "新页面" --no-create-first-block
...
| `LOGSEQ_ENABLE_ADVANCED_QUERIES` | 否 | `true` | 启用高级查询与任务工具 |
| `LOGSEQ_ENABLE_GIT_OPERATIONS` | 否 | `false` | 启用 Git 相关工具 |
| `LOGSEQ_ENABLE_ASSET_MANAGEMENT` | 否 | `false` | 预留的资源管理开关 |
...
如果 `graph git-status` 返回 `MethodNotExist`，说明当前 Logseq 构建或 API 版本不支持 Git 操作。请确认 Logseq 版本或禁用 Git 功能开关。
...
> **文档生成日期**: 2026-02-07
> **基于代码版本**: v2.1.1 (commit on main branch)
