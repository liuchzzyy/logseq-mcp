...
> **版本**: 1.0.0 | **Python**: >=3.10 | **协议**: MCP (Model Context Protocol)
>
...
本项目从 v0.0.1 的单文件架构（9 个 Tools）重构为 v1.0.0 的四层架构（25 个 Tools），重构计划详见 `plan.md`。
...
```
src/
├── __init__.py              # CLI 入口 (argparse)，40+ 子命令
...
├── services/                # 应用服务层
│   ├── __init__.py
│   ├── blocks.py            # BlockService (9 个 async 方法)
│   ├── pages.py             # PageService (5 个 async 方法)
...
### 4.2 CLI 入口 (`src/__init__.py`)

除了 MCP Server 模式，还提供了完整的命令行接口：
...
通过 `python -m src` 或 `uv run python -m src` 启动。
...
class LogseqSettings(BaseSettings):
    ...
    # 服务器元信息
    server_name: str = "mcp-server-logseq"
...
| `format_success(msg)` | 成功消息 (`✓ ...`) |
| `format_error(msg)` | 错误消息 (`✗ Error: ...`) |
...
class BlockService:
    ...
    async def update(input: UpdateBlockInput) -> BlockEntity
    async def delete(input: DeleteBlockInput) -> bool
    async def get(input: GetBlockInput) -> BlockEntity
    async def move(input: MoveBlockInput) -> BlockEntity
    async def insert_batch(input: BatchBlockInput) -> list[BlockEntity]
...
| `logseq_get_page` | src_page (必填) | 获取页面 |
...
项目提供完整的命令行界面 (`src/__init__.py`)，无需 MCP 客户端即可直接操作 Logseq。

### 命令结构

```bash
uv run python -m src [--api-key KEY] [--url URL] <command> <action> [options]
```
...
# 页面操作
uv run python -m src pages list [--repo NAME]
uv run python -m src pages get --name "页面名" [--children]
uv run python -m src pages create --name "新页面" [--properties '{"key":"val"}'] [--journal]
uv run python -m src pages delete --name "页面名"
uv run python -m src pages rename --old "旧名" --new "新名"
...
uv run python -m src blocks insert --parent "页面名" --content "内容" [--as-page-block]
uv run python -m src blocks update --uuid <uuid> --content "新内容"
uv run python -m src blocks delete --uuid <uuid>
uv run python -m src blocks move --uuid <uuid> --target <target_uuid> [--as-child]
uv run python -m src blocks batch-insert --parent "页面名" --file blocks.json
uv run python -m src blocks page-blocks --page "页面名"
uv run python -m src blocks current-page-blocks
uv run python -m src blocks current-block

# 查询操作
uv run python -m src queries simple --query "[[标签]]"
uv run python -m src queries advanced --query "[:find ...]" [--inputs '[...]']
...
> **文档生成日期**: 2026-02-07
> **基于代码版本**: v1.0.0 (commit on main branch)
